        ...
        qafDocument.getScenarios().forEach((scenario, index_s_of_qaf) => {
            if (scenario.getHasDP()) {
                ...
                scenario.tableBody?.forEach((tableRow, index_tb) => {
                    ...
                    let uri = gherkinDocumentUtil_1.GherkinDocumentUtil.getUriFromGherkinDocument(obj), name = scenario.getScenarioName(), language = "en", astNodeIdsForSteps = [], types = [], texts = [], tagNames = [], astNodeIds = [], ids = [];
                    ...
                    pickles.push(gherkinDocumentUtil_1.GherkinDocumentUtil.genPickle(uri, name, language, astNodeIdsForSteps, types, texts, tagNames, astNodeIds, ids));
                });
            }
        });
        return pickles;
